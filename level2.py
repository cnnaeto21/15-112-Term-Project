##############################
# level2.py Citation comment
# Line 21 to 269 Original code
# 270 code taken from stackoverflow link bewlow
# # https://stackoverflow.com/questions/6339057/draw-a-transparent-rectangle-in-pygame
# Line 300 and 327 taken from stackOverflow link below
...
def init(data):
    ...
    data.winnerScreen = WinnerScreen(data.width,data.height, data.score, data.timeLeft, 3)
...
def timerFiredMovement(data):
    if data.timeLeft == 0 and data.score < 20:
        data.gameOver = True
        data.outtaTime = True
...
def keyPressed(data, key):
    ...
    if data.isPaused == True:
        ...
        return 'Level 2'
def reDrawAllLevel2(data,screen):
    ...
    levelName = data.scoreFont.render("LEVEL 2", False, (0,0,0))
